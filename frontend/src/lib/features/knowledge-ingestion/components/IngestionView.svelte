...
<div class="relative flex flex-col gap-6">
	<div class="pointer-events-none absolute inset-0 opacity-20 grayscale">
		...
		<div class="flex flex-col gap-6 lg:flex-row lg:items-start">
			<Card.Root class="lg:sticky lg:top-4 lg:w-64 lg:shrink-0">
				<Card.Header>
					...
					<Card.Description>
						{store.phases.filter((p) => p.status === PhaseStatus.DONE).length} / {store.phases
							.length} completadas
					</Card.Description>
				</Card.Header>
				<Card.Content>
			...
			<div class="mx-auto flex w-full max-w-2xl min-w-0 flex-1 flex-col gap-5">
				<ChatBubble role="agent">
					...
					{/snippet}
					<p>
						Cuéntame qué fuentes quieres incorporar: archivos PDF, Excel o Markdown, o un endpoint
						de API. Cuando estés listo, envíalas y te muestro cada fase a medida que avanza.
					</p>
				</ChatBubble>
